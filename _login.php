		...
		else
		{
			if(isset($_POST['login']))
			{
				...
				if($_POST['log_captcha'] == $bchan_captcha)
				{
					...
					$sql_check_user = "SELECT * FROM bchan_user WHERE bchan_email='$login_email' AND bchan_password='$login_password'";
					$check_user = $bchan_class->q($conn, $sql_check_user);
					$fetch_user = mysqli_fetch_assoc($check_user);
					$user_email = $fetch_user['bchan_email'];
					$user_pass = $fetch_user['bchan_password'];
					...
					if($login_email == $user_email && $login_password == $user_pass)
					{
						...
						<div class="p-1">
							...
							<h4 class="text-dark bchan-font-header">You are logged in, please wait , you will be redirecting to home.</h4>
					...
					else
					{
						...
						<div class="container p-3 bg-danger text-light">
							<h6 class="bchan-font-header">Username or Password wrong!</h6>
						</div>
						<?php
					}
				}
			...
			else
			{
				?>
				<div class="p-1">
					<h3 class="display bchan-font-header">Login to Bachan Forum</h3>
